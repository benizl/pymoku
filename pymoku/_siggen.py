...
_siggen_reg_hdl = [
	...
	('out1_waveform',	REG_SG_WAVEFORMS,	lambda s, old: (old & ~0x70) | (s << 4) if s in
												[SG_WAVE_SINE, SG_WAVE_SQUARE, SG_WAVE_TRIANGLE, SG_WAVE_DC, SG_WAVE_PULSE] else None,
											lambda rval: (rval & 0x70) >> 4),
	('out2_waveform',	REG_SG_WAVEFORMS,	lambda s, old: (old & ~0x700) | (s << 8) if s in
												[SG_WAVE_SINE, SG_WAVE_SQUARE, SG_WAVE_TRIANGLE, SG_WAVE_DC, SG_WAVE_PULSE] else None,
											lambda rval: (rval & 0x700) >> 8),
	('out1_clipsine',	REG_SG_WAVEFORMS,	lambda s, old: (old & ~0x80) | (int(s) << 7) if int(s) in [0, 1] else None,
											lambda rval: (rval & 0x80) >> 7),
	('out2_clipsine',	REG_SG_WAVEFORMS,	lambda s, old: (old & ~0x800) | (int(s) << 11) if int(s) in [0, 1] else None,
											lambda rval: (rval & 0x800) >> 11),
	...
	('mod1_amplitude',	REG_SG_MODA1,		lambda a, old: _usgn(a / _SG_DEPTHSCALE, 32), lambda rval: rval * _SG_DEPTHSCALE),
	('mod2_amplitude',	REG_SG_MODA2,		lambda a, old: _usgn(a / _SG_DEPTHSCALE, 32),	lambda rval: rval * _SG_DEPTHSCALE),
